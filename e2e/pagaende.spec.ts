...
test("starten leder rakt in i den operativa vyn", async ({ page }) => {
  ...
  await expect(page.getByText("Pågår")).toBeVisible();
  await expect(page.getByText("Områdessök").first()).toBeVisible();
  await expect(page.getByText("Tid på plats")).toBeVisible();
  // Klockan har just startat: noll minuter, inte "ingen varaktighet".
  await expect(page.getByText(/^0 min/)).toBeVisible();
  await expect(page.getByText("Beräknad total tid")).toBeVisible();
  // Ekipaget som faktiskt är på plats, inte bara uppdraget.
  await expect(page.getByText(/Erik Andersson/).first()).toBeVisible();
  await expect(page.getByText("Terminal 5, Bagagehall").first()).toBeVisible();
});

test("en markering registreras med ett tryck", async ({ page }) => {
  await startatUppdrag(page, `Markering ${Date.now()}`);

  await page.getByRole("button", { name: /^Markering/ }).click();
  await expect(page.getByText("Registrerat (1)")).toBeVisible();

  await page.getByRole("button", { name: /^Fynd/ }).click();
  await expect(page.getByText("Registrerat (2)")).toBeVisible();
...
test("genomsökt andel och checklista räknas upp", async ({ page }) => {
  ...
  await expect(page.getByText("Checklista 0/6")).toBeVisible();
  await page.getByRole("button", { name: "Säkerhetsgenomgång" }).click();
  await expect(page.getByText("Checklista 1/6")).toBeVisible();
  await page.getByRole("button", { name: "Utrustning kontrollerad" }).click();
  await expect(page.getByText("Checklista 2/6")).toBeVisible();
